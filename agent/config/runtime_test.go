...
func TestLoad_IntegrationWithFlags(t *testing.T) {
	...
	run(t, testCase{
		...
		hcl: []string{`
			  ...
			  server = true
			`},
		expectedWarnings: []string{"if auto_encrypt.allow_tls is turned on, verify_incoming_rpc should be enabled. It is necessary to turn it off during a migration to TLS, but it should definitely be turned on afterwards."},
		expected: func(rt *RuntimeConfig) {
			rt.DataDir = dataDir
			rt.AutoEncryptAllowTLS = true
			rt.ConnectEnabled = true
